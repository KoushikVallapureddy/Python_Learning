...
The function should use the filter() function along with a lambda function to select strings that have a length greater than 5. 
The function should return a list containing the selected strings.
'''
def get_long_strings(strings):
    # Use filter() with a lambda function to select strings with length greater than 5
    long_strings = filter(lambda w: len(w)>5, strings)
    
    # Return the list of selected strings
    return list(long_strings)

strings = ['cat', 'dog', 'fish', 'bird']
print(get_long_strings(strings))
...
numbers = [2,3,4,5,10,11,13,17,18,19]
